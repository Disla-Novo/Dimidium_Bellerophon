...
function generateConfig() {
  ...
  const kinematics = kinematicsEl ? kinematicsEl.value : "cartesian";
  const isCoreXY = kinematics === "corexy";
  ...
  // On CoreXY, stepper_x/stepper_y no longer drive independent axes - Klipper's
  // corexy kinematics module combines both motors' motion to produce X/Y
  // movement (the classic "Motor A" / "Motor B" naming), so the two steppers
  // are called out here rather than left looking like plain independent axes.
  // The step/dir/enable pins themselves are still whatever the board wires to
  // those two motor connectors - we can't know actual belt routing generically,
  // so dir_pin polarity isn't guessed at; verify motor direction on first move
  // and flip dir_pin (add/remove the leading "!") if X/Y move opposite to
  // expected.
  cfg += `[stepper_x]\n`;
  if (isCoreXY) {
    cfg += `# CoreXY Motor A - combines with stepper_y to produce X/Y motion\n`;
  }
  ...
  cfg += `[stepper_y]\n`;
  if (isCoreXY) {
    cfg += `# CoreXY Motor B - combines with stepper_x to produce X/Y motion\n`;
  }
...
function saveSession() {
  ...
  const formData = {
    ...
    kinematics: kinematicsEl ? kinematicsEl.value : "cartesian",
...
function loadSession() {
  ...
  try {
    ...
    if (data.kinematics) {
      const kinematicsEl = document.getElementById("kinematics");
      if (kinematicsEl) kinematicsEl.value = data.kinematics;
    }
